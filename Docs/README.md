# Video Editor Documentation

Documentation for the native macOS video editor.

## Product
- PRD.md
- PRODUCT_ROADMAP.md
- DESIGN_SYSTEM.md

## Architecture
- ARCHITECTURE.md
- TECH_STACK.md
- PROJECT_STRUCTURE.md
- MODULE_SYSTEM.md
- MODULE_API.md
- DATA_MODEL.md
- COMMAND_ARCHITECTURE.md
- RENDER_ARCHITECTURE.md
- UNDO_REDO.md
- PROJECT_LIFECYCLE.md
- FILE_SYSTEM.md

## Media & Editing
- MEDIA_MANAGEMENT.md
- PROJECT_FORMAT.md
- TIMELINE_ENGINE.md
- PLAYBACK_ENGINE.md
- EXPORT_PIPELINE.md
- CACHE_SYSTEM.md

## UI & Interaction
- UI_GUIDELINES.md
- MENU_STRUCTURE.md
- KEYBOARD_SHORTCUTS.md
- COMMANDS.md
- SETTINGS.md
- LOCALIZATION.md

## Reliability & Platform
- ERROR_HANDLING.md
- COMPATIBILITY.md
- PERFORMANCE.md
- ACCESSIBILITY.md
- PRIVACY.md
- SECURITY.md
- OBSERVABILITY.md

## Engineering & Release
- TESTING.md
- DEVELOPMENT.md
- VERSIONING.md
- RELEASE_PROCESS.md
- CONTRIBUTING.md
- CHANGELOG.md

## Module Roadmap

The product defines 14 optional modules.

### v1.0 — 6 modules
1. Effects
2. Transitions
3. Advanced Subtitles
4. Professional Color
5. Audio Pro
6. Advanced Export

### Future — 8 modules
7. Motion Graphics
8. Pro Media / Codecs
9. Masking & Tracking
10. AI Tools
11. Advanced Proxy
12. Social Media Presets
13. Templates
14. Integrations

## Swift / Xcode Implementation

The documentation also defines the implementation blueprint for the native Swift application:
- SWIFT_ARCHITECTURE.md
- SWIFTUI_ARCHITECTURE.md
- CORE_SERVICES.md
- TIMELINE_DATA_STRUCTURE.md
- MEDIA_PIPELINE.md
- MEDIA_INDEXING.md
- RENDER_ARCHITECTURE.md
- EXPORT_ARCHITECTURE.md
- MODULE_RUNTIME.md
- PROJECT_FILE_FORMAT.md
- APP_LIFECYCLE.md
- MACOS_INTEGRATION.md

## Product Identity

**Kino** is the working product name for this native macOS video editor.

The product name should be written as **Kino** in documentation, UI copy, project metadata and branding.
